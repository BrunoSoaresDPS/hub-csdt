  ...
  if (projects.length === 0) {
    return (
      <div className="iveco-card flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm font-medium text-gray-500 dark:text-[#555562]">Nenhum projeto encontrado</p>
        <p className="mt-1 text-xs text-gray-400 dark:text-[#333340]">Tente ajustar os filtros ou aguarde novos cadastros.</p>
      </div>
    );
  }
    ...
    <div className="iveco-card overflow-hidden">
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50 dark:border-[#232329] dark:bg-[#17171b]">
              {['Projeto', 'Categorias', 'Responsável', 'Status', 'Prioridade', 'Complexidade', ''].map((h) => (
                <th key={h} className="px-5 py-3.5 text-xs font-semibold uppercase tracking-[0.1em] text-gray-400 dark:text-[#555562]">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-[#17171b]">
            {projects.map((project) => (
              <tr
                key={project.id}
                className="group transition-colors hover:bg-gray-50 dark:hover:bg-[#131316]"
              >
                <td className="px-5 py-4">
                  <p className="font-semibold text-gray-900 dark:text-white">{project.title}</p>
                  <p className="mt-0.5 text-xs text-gray-400 dark:text-[#555562]">
                    ...
                    try {
                      ...
                      return (
                        <div className="flex flex-wrap gap-1">
                          {shown.map((cat: string) => (
                            <span key={cat} className="inline-flex rounded-full border border-[#1654FF]/25 bg-[#1654FF]/10 px-2.5 py-0.5 text-xs font-medium text-[#1654FF] dark:text-[#7B9FFF]">
                              {cat}
                            </span>
                          ))}
                          {rest > 0 && (
                            <span className="inline-flex rounded-full border border-[#1654FF]/25 bg-[#1654FF]/10 px-2.5 py-0.5 text-xs font-medium text-[#1654FF] dark:text-[#7B9FFF]">
                              +{rest} mais
                            </span>
                          )}
                        </div>
                      ...
                      return <span className="text-gray-400 dark:text-[#9999a8]">—</span>;
                    }
                  })()}
                </td>
                <td className="px-5 py-4 text-gray-500 dark:text-[#9999a8]">{project.owner}</td>
                <td className="px-5 py-4">
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-500 dark:text-[#9999a8]">
